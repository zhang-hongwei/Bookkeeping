...
- Your task is to review submitted Git diffs and return a structured review report.
...
4. Use `read_file` to read [rules-attach.md](.claude/rules/rules-attach.md). Even if you think it's unnecessary, you must read it.
...
read [typescript.md](.claude/rules/typescript.md) for the consolidated project code style and optimization rules.

### Code Optimization

The optimization checklist has been consolidated into [typescript.md](.claude/rules/typescript.md): loops, debouncing/throttling, design system components, theming tokens, concurrency with `Promise.*`, minimal DB column selection, and package reuse.
