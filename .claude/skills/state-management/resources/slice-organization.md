# Zustand Store Slice 组织架构

本文档描述了 Project 项目中 Zustand Store 的模块化 Slice 组织方式，展示如何通过分片架构管理复杂的应用状态。

## 顶层 Store 结构

Project 的 `chat` store (`src/store/chat/`) 采用模块化的 slice 结构来组织状态和逻辑。
