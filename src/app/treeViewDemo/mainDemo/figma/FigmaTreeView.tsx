...
import { ExtendedTreeItemProps, IdType, ItemType } from './items';
...
const CustomTreeItem = React.forwardRef(function CustomTreeItem(
  ...
  const { id, itemId, label, disabled, children, onAddNode, onDeleteNode, ...other } = props;
      ...
      >
        <CustomTreeItemContent {...getContentProps()}>
          ...
          <Stack direction="row" spacing={0.5} alignItems="center">
            ...
            <Tooltip title={status.disabled ? '解锁' : '锁定'} arrow>
              <IconButton
                size="small"
                onClick={(e) => {
                  e.stopPropagation();
...
type FigmaTreeViewProps = {
  ...
  setItems: React.Dispatch<React.SetStateAction<TreeViewBaseItem<ExtendedTreeItemProps>[]>>;
};
...
export default function FigmaTreeView({
  ...
  // 递归添加节点
  const addNodeToTree = (
    tree: TreeViewBaseItem<ExtendedTreeItemProps>[],
    parentId: string,
    newNode: TreeViewBaseItem<ExtendedTreeItemProps>,
  ...
  const handleAddNode = (itemId: string) => {
    setTargetItemId(itemId);
    setDialogOpen(true);
  };

  const handleDeleteNode = (itemId: string) => {
    setItems((prevItems) => deleteNodeFromTree(prevItems, itemId));
    if (selectedItem === itemId) {
      setSelectedItem(null);
    }
  };

  const handleDialogConfirm = (nodeType: ItemType, label: string) => {
    if (targetItemId) {
      const newNode: TreeViewBaseItem<ExtendedTreeItemProps> = {
        id: generateUniqueId(),
        label,
        itemType: nodeType,
      };
      setItems((prevItems) => addNodeToTree(prevItems, targetItemId, newNode));
    }
  };
