/**
 * Clip Path Export Dialog
 * Export clip paths in various formats
 */
...
interface ClipPathExportDialogProps {
  open: boolean;
  config: ClipPathConfig;
  onClose: () => void;
}

const FORMATS: { label: string; value: ClipPathExportFormat }[] = [
  { label: "CSS", value: "css" },
...
export function ClipPathExportDialog({
  open,
  config,
  onClose,
}: ClipPathExportDialogProps) {
              ...
              <TabPanel key={format.value} value={tabValue} index={index}>
                <Box sx={{ position: "relative" }}>
                  <TextField
                    ...
                    fullWidth
                    rows={8}
                    value={code || "/* No clip path applied */"}
                    InputProps={{
                      readOnly: true,
                      sx: {
