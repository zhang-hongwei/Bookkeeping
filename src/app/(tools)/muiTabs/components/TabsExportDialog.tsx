/**
 * MUI Tabs Theme Designer - Export Dialog
...
import type { TabsThemeConfig, TabsThemeExportFormat } from "../types";
import { exportConfig } from "../utils";

interface TabsExportDialogProps {
  open: boolean;
  config: TabsThemeConfig;
  onClose: () => void;
}

export function TabsExportDialog({
  open,
  config,
  onClose,
}: TabsExportDialogProps) {
  const [format, setFormat] = React.useState<TabsThemeExportFormat>("createTheme");
    ...
    <Dialog open={open} onClose={onClose} maxWidth="lg" fullWidth>
      <DialogTitle>
        <Stack direction="row" alignItems="center" justifyContent="space-between">
          <Typography variant="h6">Export Tabs Theme</Typography>
      ...
      <DialogContent>
        <MuiTabs
          value={format}
          onChange={(_, newValue) => setFormat(newValue as TabsThemeExportFormat)}
