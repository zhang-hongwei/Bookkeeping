...
export const useReadOnlyStyles = (editorRef: EditorRefType) => {
  // Use ref to persist decoration IDs across renders
  ...
  useEffect(() => {
    ...
    return () => {
      editorRef.current?.deltaDecorations(decorationIdsRef.current, []) // wipe any existing decorations
      modelContentChangeBinding?.dispose()
    }
    // applyStyles uses decorationIdsRef which is stable, editorRef for cleanup
