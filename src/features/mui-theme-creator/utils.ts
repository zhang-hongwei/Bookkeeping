...
import type { ThemeCreatorStore } from "@/store/mui-theme-creator/types"
...
export const removeByPath = (object: any, path: string): any => {
...
export const setByPath = (object: any, path: string, value: any): any =>
 ...
 */
export const generateThemeId = (savedThemes: ThemeCreatorStore["savedThemes"]): string => {
 ...
 * Uses JSON5 for safe parsing without code execution
 */
export const parseEditorOutput = (code: string): any => {
  try {
    // Extract the themeOptions object from the code
    // Expected format: export const themeOptions: ThemeOptions = { ... };
    const match = code.match(/export\s+const\s+themeOptions\s*:\s*ThemeOptions\s*=\s*({[\s\S]*?});?\s*$/m);

    if (!match || !match[1]) {
      throw new Error('Could not find themeOptions export in code');
    }

    // Use JSON5 to parse the object literal safely
    const JSON5 = require('json5');
    return JSON5.parse(match[1]);
  } catch (error: any) {
    throw new Error('Failed to parse editor output: ' + error.message);
  }
};
