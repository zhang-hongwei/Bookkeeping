...
export const getAuthConfig = () => {
  ...
  if (process.env.AUTH0_CLIENT_SECRET) {
    console.warn(
      removeTipsTemplate("AUTH0_CLIENT_SECRET", "AUTH_AUTH0_SECRET")
    );
  }
  ...
  if (process.env.AUTHENTIK_CLIENT_ID) {
    console.warn(
      removeTipsTemplate("AUTHENTIK_CLIENT_ID", "AUTH_AUTHENTIK_ID")
    );
  }
  if (process.env.AUTHENTIK_CLIENT_SECRET) {
    console.warn(
      removeTipsTemplate("AUTHENTIK_CLIENT_SECRET", "AUTH_AUTHENTIK_SECRET")
    );
  }
  if (process.env.AUTHENTIK_ISSUER) {
    console.warn(
      removeTipsTemplate("AUTHENTIK_ISSUER", "AUTH_AUTHENTIK_ISSUER")
    );
  }
  ...
  if (process.env.AUTHELIA_CLIENT_SECRET) {
    console.warn(
      removeTipsTemplate("AUTHELIA_CLIENT_SECRET", "AUTH_AUTHELIA_SECRET")
    );
  }
  ...
  if (process.env.AZURE_AD_CLIENT_SECRET) {
    console.warn(
      removeTipsTemplate("AZURE_AD_CLIENT_SECRET", "AUTH_AZURE_AD_SECRET")
    );
  }
  if (process.env.AZURE_AD_TENANT_ID) {
    console.warn(
      removeTipsTemplate("AZURE_AD_TENANT_ID", "AUTH_AZURE_AD_TENANT_ID")
    );
  }
  ...
  if (process.env.GENERIC_OIDC_CLIENT_ID) {
    console.warn(
      removeTipsTemplate("GENERIC_OIDC_CLIENT_ID", "AUTH_GENERIC_OIDC_ID")
    );
  }
  ...
  if (process.env.GENERIC_OIDC_ISSUER) {
    console.warn(
      removeTipsTemplate("GENERIC_OIDC_ISSUER", "AUTH_GENERIC_OIDC_ISSUER")
    );
  }
  ...
  if (process.env.GITHUB_CLIENT_SECRET) {
    console.warn(
      removeTipsTemplate("GITHUB_CLIENT_SECRET", "AUTH_GITHUB_SECRET")
    );
  }
  ...
  if (process.env.LOGTO_CLIENT_SECRET) {
    console.warn(
      removeTipsTemplate("LOGTO_CLIENT_SECRET", "AUTH_LOGTO_SECRET")
    );
  }
  ...
  if (process.env.ZITADEL_CLIENT_SECRET) {
    console.warn(
      removeTipsTemplate("ZITADEL_CLIENT_SECRET", "AUTH_ZITADEL_SECRET")
    );
  }
  ...
  return createEnv({
    client: {
      ...
      /**
       * whether to enable clerk (can be controlled by NEXT_PUBLIC_ENABLE_CLERK)
       */
      NEXT_PUBLIC_ENABLE_CLERK: z.string().optional(),
    ...
    runtimeEnv: {
      ...
      NEXT_PUBLIC_ENABLE_CLERK_AUTH:
        ...
        process.env.NEXT_PUBLIC_ENABLE_CLERK !== "0" &&
        !!process.env.NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY,
      NEXT_PUBLIC_ENABLE_CLERK: process.env.NEXT_PUBLIC_ENABLE_CLERK,
      ...
      NEXT_PUBLIC_ENABLE_NEXT_AUTH:
        process.env.NEXT_PUBLIC_ENABLE_NEXT_AUTH === "true",
      ...
      // Cloudflare Zero Trust
      CLOUDFLARE_ZERO_TRUST_CLIENT_ID:
        process.env.CLOUDFLARE_ZERO_TRUST_CLIENT_ID,
      CLOUDFLARE_ZERO_TRUST_CLIENT_SECRET:
